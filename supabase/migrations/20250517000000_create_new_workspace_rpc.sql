-- Atomic workspace bootstrap for authenticated users (avoids RLS edge cases on chained inserts).
-- Creates workspace (owner membership via trigger), default app, and a sample listing_generation row.
...
comment on function public.create_new_workspace(text) is
  'Creates a workspace for auth.uid(), default app, sample listing_generation; returns workspace id.';
