-- Site admin flag + SECURITY DEFINER RPC for read-only financial aggregates.
-- Admins are granted via SQL: update public.profiles set is_admin = true where id = '<uuid>';

alter table public.profiles
  add column if not exists is_admin boolean not null default false;

comment on column public.profiles.is_admin is
  'When true, user may call admin_financial_snapshot and access /admin routes (defense in depth with app middleware).';
