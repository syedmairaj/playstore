...
function averageRankLastDays(
  ranks: { rank: number | null; captured_at: string }[],
  days: number,
): number | null {
  const cutoff = Date.now() - days * MS_DAY;
  const nums = ranks
    .filter((r) => r.rank != null && new Date(r.captured_at).getTime() >= cutoff)
    .map((r) => r.rank as number);
  if (nums.length === 0) return null;
  return nums.reduce((a, b) => a + b, 0) / nums.length;
}

function ranksInWindow(
  ranksChrono: { rank: number | null; captured_at: string }[],
  days: RangeDays,
...
function trendTone(
  ...
  const windowed = ranksInWindow(ranksChrono, days).filter((r) => r.rank != null);
...
export function KeywordHistoryDialog({
  ...
  const rankMarketChips = useMemo(
    ...
    [data?.market, appForKeyword?.target_countries],
  );
  ...
  const chartSeries = useMemo(() => {
    const windowed = ranksInWindow(ranksChrono, range).filter((r) => r.rank != null);
        ...
        <div className="space-y-8 px-6 py-6 sm:px-8 sm:py-7">
              ...
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={chartSeries} margin={{ top: 16, right: 8, left: 4, bottom: 12 }}>
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ fontSize: 11, color: "#a1a1aa", marginBottom: 4 }}
                    labelFormatter={(label, payload) => {
                      const row = payload?.[0]?.payload as { captured_at?: string } | undefined;
                      if (row?.captured_at) {
                        return dateFmt.format(new Date(row.captured_at));
                      }
                      return String(label ?? "");
                    }}
                    formatter={(value) =>
                      typeof value === "number"
                        ? [formatRankForDisplay(value as number, rankFmt), t("columns.rank")]
                        : [String(value ?? ""), t("columns.rank")]
                    }
                  />
                  <Line
                    type="monotone"
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
            ...
            <div className="rounded-2xl border border-emerald-500/15 bg-gradient-to-b from-emerald-500/[0.06] to-transparent px-4 py-3.5 shadow-sm ring-1 ring-inset ring-emerald-500/10 sm:px-5 sm:py-4">
              ...
              <p className="mt-2 font-mono text-xl font-semibold tabular-nums tracking-tight text-emerald-300">
                {best != null ? formatRankForDisplay(best, rankFmt) : "—"}
              </p>
            </div>
            <div className="rounded-2xl border border-white/[0.06] bg-gradient-to-b from-white/[0.04] to-transparent px-4 py-3.5 shadow-sm ring-1 ring-inset ring-white/[0.04] sm:px-5 sm:py-4">
                    ...
                    {tableRows.map(({ row, deltaKind, deltaAbs }) => (
                      <tr key={`${row.captured_at}-${row.rank ?? "x"}`} className="border-b border-white/[0.04]">
                        ...
                        <td className="px-4 py-2.5 text-end font-mono text-emerald-200/90">
                          {row.rank != null ? formatRankForDisplay(row.rank, rankFmt) : "—"}
                        </td>
