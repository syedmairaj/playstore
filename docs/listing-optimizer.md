...
1. **Browser** — `app/[locale]/app/[workspaceId]/listing-optimizer/page.tsx` renders `components/ListingOptimizer.tsx` with a `workspaceId`. Legacy `/optimizer` redirects to `/listing-optimizer`. The form POSTs JSON to the Next.js route handler (cookies send the user session).
2. **API**
   - **`POST /api/listings/generate`** (`app/api/listings/generate/route.ts`):
     ...
     - **Validate model output** — Zod `listingGenerationOutputSchema` enforces Play Store–friendly length caps (title max **30**, short **80**, long **4000**). Before validation, the server **hard-clamps** `title`, `shortDescription`, and `fullDescription` to those max lengths (word-aware when possible for title/short, then ellipsis if still over) so minor model overruns do not fail the request.
...
- **Optimizer subtree:** `components/ListingOptimizer.tsx` wraps the screen in `dir="rtl"` when `locale === "ar"`, applies Tailwind `font-arabic` (Noto + Geist fallback), and passes `previewDir` into `LivePreviewPhone` so in-phone copy mirrors RTL. Prefer logical spacing (`ps`/`ms`, `text-end`, `start`/`end` in Tailwind) in optimizer-related UI.
...
- **Versioning** — `getListingOptimizerPromptVersion()` in `lib/prompts/listing-optimizer.ts` (e.g. `listing-optimizer-v3`). Stored on each row for traceability.
