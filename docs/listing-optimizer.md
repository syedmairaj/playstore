...
**Active Context Competitive Defense (ASO multi-app):** Market Intelligence Signals come only from Market Intel staging (`source: market_intel`, same selected workspace app). Competitor Strengths shows audit-approved `competitor_strength` (ACTIVE) **and** Competitor Spy keyword gaps / quick wins (`competitor_keyword` with `from_gap_analysis` — UI must accept typed `keyword_gap` normalization). Review weaknesses use **Stage Issue** on Reviews → Competitive Exploitation (lands in **Review Insights / Conversion Uplift** immediately — including oppositional targets). `mergeStagedQueueIntoAdopted` must only mirror `review_pain_point` / `feature_request` rows into Review Insights — never gap keywords (`explicitly_staged` alone is not enough). Competitor Spy **Queue Spy sentiment terms** → Review Insights only. Gap CTA: **Queue gap keywords** → Competitor Strengths. The Competitor Spy “Signals Queued” panel is a shared inbox for all signal types, not Review Insights alone.

**Market Discovery Auto-Fill (research mode):** There is no separate “Scan” control. Paid research is **Analyze Competitor** on Competitor Spy. Auto-Fill fills Target Keywords / App Features from **Active Context** (queued gap keywords, quick wins, market intel, review insights) via `buildDiscoveryFieldsFromActiveContext` — auto once per app when fields are empty, or via **Pull from Active Context** / ✦ in recommended mode. **Start from Scratch** uses credit ✦ autofill (`POST /api/listings/optimizer-autofill`). Opening Optimizer from Competitor Spy toasts may also inject keyword context via `setPlaystoreInjectedKeywordContext`.

**Build listing draft (instant draft):** `POST /api/listings/generate` with `isDraft: true` returns a Core ASO template (no credit debit). The client must apply the response into `result` + modular panel state, set wizard/`serverStep` to Final Optimization, and must not clear the prior result until the new draft is applied — otherwise the results area flashes empty after a long request. Dev remounts (RSC refresh / HMR) during the POST are common (~60s requests); the client broadcasts `playstore:optimizer-instant-draft-ready` and writes a final-listing cache with `allowInstantDraftRestore` so the live Optimizer instance can re-apply after remount.
2. **API**
...
- **Modular pipeline (v1)** — `POST /api/listings/generate` accepts `generationStep`: `title` | `short` | `long` | `hook` | `features` | `closing` | `finalize` | `full`. Modular steps run **without** debiting credits; **`finalize`** (and legacy **`full`**) debit `listing_generation` (5 credits), persist to `listing_generations`, and run Context Audit + queue-hash validation on every step. Client hook: `src/hooks/useModularGeneration.ts`; UI panel: `modular-listing-panel.tsx`. State shape: `{ title: { value, locked }, shortDescription: { variations, selectedIndex }, longDescription: { hook, features, closing } }`.
- **Versioning** — `getListingOptimizerPromptVersion()` in `lib/prompts/listing-optimizer.ts` (e.g. `listing-optimizer-v16.0`). Stored on each row for traceability.
- **Orchestration Protocol (v16)** — Three-phase synthesis returned as discrete JSON modules (`orchestration.modules.anchor`, `.conversion`, `.expansion`) alongside legacy root fields (`title`, `shortDescription`, `fullDescription`). Phase 1 builds a hybrid title (user-locked keywords + AI suggestions) and `keywordAnchor`; Phase 2 emits three short-description variations (defensive / offensive / primary) from Strategy Profile + review insights; Phase 3 structures the long description into hook / emoji feature bullets / trust CTA blocks. Schemas: `lib/listing/orchestration-protocol.schema.ts`; prompt block: `lib/prompts/listing-orchestration-protocol.ts`; server maps modules onto root fields via `lib/listing/apply-orchestration-output.ts`. The results UI (`orchestration-modules-panel.tsx`) shows each phase independently and supports per-module regenerate (client merges preserved modules after a scoped `userInstruction` run).
