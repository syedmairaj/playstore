...
export default async function GrowthPushTeaserPage({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-8 pb-8">
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-8 shadow-lg shadow-black/20">
        <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#60A5FA]/90">
          {t("eyebrow")}
        </p>
        <h1 className="mt-3 text-2xl font-semibold tracking-tight text-white">{t("push.title")}</h1>
        <p className="mt-4 text-sm leading-relaxed text-white/55">{t("push.body")}</p>
        <ul className="mt-6 space-y-2 text-sm text-white/45">
          <li className="flex gap-2">
            <span className="text-[#60A5FA]/80">✓</span>
            {t("push.bullet1")}
          </li>
          <li className="flex gap-2">
