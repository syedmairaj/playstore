...
import { generateAddAppFieldSuggest } from "@/lib/gemini/generate-add-app-suggest";
...
import { addAppSuggestBodySchema } from "@/lib/validation/add-app-suggest-body";
import { getWorkspaceRole } from "@/lib/workspace/membership";

const ROUTE = "POST /api/apps/suggest";
...
export async function POST(request: NextRequest) {
  ...
  let input;
  try {
    input = addAppSuggestBodySchema.parse(body);
  } catch (e) {
  ...
  const { workspaceId, field, context } = input;
  ...
  const creditCost = AI_CREDIT_COSTS.add_app_field_suggest;
  ...
  const debit = await consumeWorkspaceAiCredits(supabase, {
    ...
    userId: user.id,
    amount: creditCost,
    description: `Add app AI suggest (${field})`,
    sourceType: "generation",
    meta: { route: ROUTE, tool: "add_app_field_suggest", model, field },
  });
  ...
  try {
    const text = await generateAddAppFieldSuggest({ field, context });
    await logUsage(admin, {
      route: ROUTE,
      clientIp,
      success: true,
    ...
    if (ledgerId) {
      await refundWorkspaceAiCredits(supabase, {
        ledgerId,
        userId: user.id,
        reason: "Add app field suggest failed before a saved result",
      });
    }
    const message =
      e instanceof Error ? e.message : "Suggestion failed unexpectedly";
    await logUsage(admin, {
      route: ROUTE,
      clientIp,
      success: false,
